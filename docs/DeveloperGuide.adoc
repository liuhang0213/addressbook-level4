...
==== Implementation
Each successful `SearchF` command execution with non-zero search results, activates an update of search count for everyone in search results.+
[source,java]
----
...
`Search` lets users search for contacts in the address book by keywords in their contact details such as name, address and telephone number +
